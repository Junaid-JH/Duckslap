...
window.addEventListener("load", function () {
    setTimeout(function(){alert("Welcome to Duckslap! Ducks are attacking you and you have to slap away! The more you slap, the faster they come at you! Click on any duck to start the game!")}, 100);
});
...
document.querySelectorAll(".duck-container > div").forEach(function(element){element.setAttribute("onclick", "initiateGame()")});

//Removes 1st onClick event in preparation for 2nd onClick event and calls the function that starts the game
function initiateGame(){
document.querySelectorAll(".duck-container > div").forEach(function(element){element.removeAttribute("onclick")}); 
startGame();
};
...
function startGame(){
document.querySelectorAll(".duck-container > div").forEach(function(element){element.setAttribute("onclick", "captureClick(this.getAttribute('class'))")});
...
function failure(){
 ...
 document.querySelectorAll(".waviy > span").forEach(function(element){element.style.animationPlayState = 'paused';});
setTimeout(function(){alert("Oh no! You've missed a duck! Your score is: " + score + " \nRefresh the page to try again and better your score!");}, 100);
};
